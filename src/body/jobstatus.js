...
class JobStatus extends Component {
    ...
    if (this.props.jobid !== "" && this.props.jobid !== undefined) {
      ...
      const acknowledgement_btn = (
        <Button
          size="small"
          type="primary"
          onClick={() => notification.close("data_retention_notice")}
        >
          Got it!
        </Button>
      );
      ...
      notification.warning({
        ...
        description: `Files for the job ${this.props.combinedJobIdDate} will be retained for 14 DAYS following job creation.\
  ...
  componentDidUpdate() {
    ...
    if (statuses.includes(this.state.pdb2pqr.status)) {
      //} && statuses.includes(this.state.pdb2pqr.status)){
      clearInterval(this.fetchIntervalPDB2PQR);
    }
    ...
    return this.fetchObjectHead(window._env_.OUTPUT_BUCKET_HOST, object_name)
      .then((response) => {
        if (response.ok) {
          ...
          console.log(
            "couldn't find pdb2pqr-status.json. Job is not post-PDB2PQR",
          );
    ...
    let interval = setInterval(function () {
      fetch(status_url)
        .then((response) => {
          if (!response.ok) {
            let err = Error(
              `HTTP code while fetching '${status_filename}': ${response.status}`,
            );
        ...
        .then((data) => {
          ...
          if (
            statusStates.includes(data[jobtype].status) &&
            ...
            self.getAllFileSizes(
              window._env_.OUTPUT_BUCKET_HOST,
              all_filenames,
              self.props.jobid,
              self.props.jobdate,
              jobtype,
        ...
        .catch((error) => {
          ...
          if (
            self.fetchIntervalErrorCount[jobtype] > self.fetchIntervalErrorLimit
          ) {
  ...
  usingJobDate() {
    if (this.props.jobdate !== null) return true;
    else return false;
  }
  ...
  getAllFileSizes(bucket_url, objectname_list, job_id, job_date, job_type) {
    ...
    Promise.all(promise_list).then((all_responses) => {
      let filesize_dict = {};
    ...
    let interval = setInterval(function () {
      if (self.state[jobtype].status !== "no_job") {
        ...
        if (start !== null) {
          ...
          elapsedHours = self.prependZeroIfSingleDigit(
            elapsedDate.getUTCHours().toString(),
          );
          elapsedMin = self.prependZeroIfSingleDigit(
            elapsedDate.getUTCMinutes().toString(),
          );
          elapsedSec = self.prependZeroIfSingleDigit(
            elapsedDate.getUTCSeconds().toString(),
          );
        } else {
          elapsedHours = "00";
          elapsedMin = "00";
  ...
  createFileListItem(item) {
    ...
    let action_list = [
      ...
      <a href={window._env_.OUTPUT_BUCKET_HOST + "/" + item}>
    ...
    return <List.Item actions={action_list}>{file_name}</List.Item>;
  }
        ...
        <AutoSizer disableHeight>
          {({ width }) => (
            <VirtualList
              ...
              width={width}
              rowHeight={rowHeight}
              rowRenderer={this.logRowRenderer({
                rows,
                stylesheet,
                useInlineStyles,
              })}
  ...
  renderLogFiles() {
    ...
    const createLogDownload = (filename) => {
      ...
      if (
        this.state.filesizes[this.props.jobtype][
          `${this.props.jobdate}/${this.props.jobid}/${filename}`
        ] > 0
      ) {
        return (
    ...
    if (this.state.file_sizes_retrieved) {
      ...
      if (this.props.jobtype === JOBTYPES.PDB2PQR) {
        logfile_view_pdb2pqr_only = (
          <Panel
            header={`Log (${log_filename})`}
            extra={createLogDownload(log_filename)}
          >
            {this.renderCodeBlock(
              this.state.logData.log,
              "accesslog",
              `${this.props.jobid}.log`,
            )}
          </Panel>
        );
      ...
      return (
        <div style={{ height: "100%" }}>
          <Collapse bordered={false}>
            {logfile_view_pdb2pqr_only}
            <Panel
              header={`Stdout (${stdout_filename})`}
              extra={createLogDownload(stdout_filename)}
            >
              {this.renderCodeBlock(
                this.state.logData.stdout,
                "accesslog",
                `${this.props.jobid}.stdout`,
              )}
            </Panel>
            <Panel
              header={`Stderr (${stderr_filename})`}
              extra={createLogDownload(stderr_filename)}
            >
              {this.renderCodeBlock(
                this.state.logData.stderr,
                "accesslog",
                `${this.props.jobid}.stderr`,
              )}
            </Panel>
          </Collapse>
        </div>
  ...
  renderCurrentStepTimeline() {
    ...
    if (this.state[jobtype].status === "pending") {
      timeline_list.push(
        <Timeline.Item>{this.possibleJobStates.submitted}</Timeline.Item>,
      );
      ...
      timeline_list.push(
        <Timeline.Item>{this.possibleJobStates.pending}</Timeline.Item>,
      );
      ...
      timeline_list.push(
        <Timeline.Item>{this.possibleJobStates.running}</Timeline.Item>,
      );
      timeline_list.push(
      ...
      timeline_list.push(
        <Timeline.Item color="green" dot={<CheckCircleOutlined />}>
          {this.possibleJobStates.complete}
        </Timeline.Item>,
      );
    }
        ...
        <Row justify="center">
          <Col span={22}>
            ...
            >
              ...
              <Step
                icon={pending_icon}
                title={this.possibleJobStates.pending}
              />
              <Step
                icon={running_icon}
                title={this.possibleJobStates.running}
              />
              <Step
                icon={complete_icon}
                title={this.possibleJobStates.complete}
              />
    ...
    return (
      <Layout>
        <Typography>
          ...
          <Paragraph>
            Your request URL may be missing the following fields: jobid,
            jobtype, jobdate
          </Paragraph>
          <Paragraph>
            Usage: /jobstatus?jobid=<b>JOBID</b>&amp;jobtype=<b>JOBTYPE</b>
            &amp;jobdate=<b>JOBDATE</b>
          </Paragraph>
  ...
  renderWorkflowSteps() {
    ...
    if (this.isUsingJobtype(JOBTYPES.PDB2PQR)) {
      workflow_header = (
        <WorkflowHeader currentStep={1} stepList={WORKFLOW_TYPES.PDB2PQR} />
      );
    } else if (this.isUsingJobtype(JOBTYPES.APBS)) {
      if (this.state.is_apbs_post_pdb2pqr === true) {
        workflow_header = (
          <WorkflowHeader currentStep={3} stepList={WORKFLOW_TYPES.APBS} />
        );
      } else if (this.state.is_apbs_post_pdb2pqr === false) {
        workflow_header = (
          <WorkflowHeader currentStep={1} stepList={WORKFLOW_TYPES.APBS_ONLY} />
        );
      }
    }
  ...
  renderJobStatsRow() {
    ...
    const elapsedTime =
      this.state.elapsedTime[jobtype] !== undefined
        ? this.state.elapsedTime[jobtype]
        : "computing...";
  ...
  createNextProcessButton() {
    ...
    if (jobtype === "pdb2pqr") {
      ...
      if (this.state[jobtype].status === "complete") {
        ...
        if (this.state[jobtype].files_output.some((e) => e.slice(-3) === ".in"))
          // check if APBS input file exists in output_files with *.in
          is_disabled = false;
      }
      next_process_button = (
      ...
      <Row gutter={16} justify="center">
        ...
        <Col span={11}>
          <List
            ...
            bordered
            header={
              <Title level={5}>{jobtype.toUpperCase()} Input Files</Title>
            }
            ...
            renderItem={(item) => {
              ...
              if (this.state.show_download_button) {
                action_list.push(
                  this.formatBytes(
                    this.state.filesizes[this.props.jobtype][item],
                  ),
                  <a href={window._env_.OUTPUT_BUCKET_HOST + "/" + item}>
                    <DownloadOutlined /> Download{" "}
                  </a>,
        ...
        <Col span={11}>
          <List
            ...
            bordered
            header={
              <Title level={5}>{jobtype.toUpperCase()} Output Files</Title>
            }
      ...
      <div>
        <Row justify="center">
          <Text>
            ...
            <Text strong>register your use of this software</Text>:
          </Text>
        </Row>
          ...
          <Col span={24}>
            <h2>
              ...
              <Checkbox
                ...
                onChange={(e) =>
                  this.setState({ show_log_line_numbers: e.target.checked })
                }
              >
                Show line numbers
              </Checkbox>
