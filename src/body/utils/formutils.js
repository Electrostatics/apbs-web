...
class ConfigForm extends Component {
  ...
  uploadJobFilesAzure(job_file_name, token_request_payload, upload_file_data) {
    let self = this;
    ...
    fetch(window._env_.API_TOKEN_URL, {
      ...
      .then((data) => {
        ...
        for (let file_name of Object.keys(url_table)) {
          let sas_url = url_table[file_name];
        ...
        Promise.all(fetch_list)
          .then(function (all_responses) {
            ...
            self
              .uploadFileToAzure(
                job_config_file_url,
                upload_file_data[job_file_name],
              )
              // .uploadFileToS3(
              //   job_config_file_url,
              //   upload_file_data[job_file_name],
              ...
              .then((job_upload_response) => {
                if (
                  job_upload_response.status < 200 ||
                  job_upload_response.status >= 300
                ) {
                  successful_submit = false;
                }
              });
  ...
  redirectToStatusPage(job_type) {
    ...
    const jobid = this.props.jobid || this.state.jobid;
    console.log("REDIRECT job id prop: ", this.props.jobid);
    console.log("REDIRECT job id state: ", this.state.jobid);
    return (
      <Redirect
  ...
  renderSubmitButton(button_is_clickable = true) {
    ...
    if (!this.state.job_submit) submission_text = "Start Job";
    else {
