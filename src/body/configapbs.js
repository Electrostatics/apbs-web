...
class ConfigAPBS extends ConfigForm {
    ...
    if (this.state.show_register_button) {
      return (
        <div>
          ...
          <br/> */}
          <Form.Item label="For continued support of this server, please register your use of this software:">
            <a
    ...
    if (self.state.job_submit)
      ...
      if (self.props.jobid) {
        ...
        for (let file of [].concat(
          self.state.infileList,
          self.state.readfileList,
        )) {
    ...
    for (let line_num in read_section_lines) {
      ...
      switch (keyword) {
        ...
        case "fglen":
          ...
          break;
        case "pdie":
          infile_json.biomolecularDielectricConstant = parseFloat(
            line_contents[0],
          );
          break;
        case "sdie":
          ...
          break;
        case "sdens":
          infile_json.surfaceConstructionResolution = parseFloat(
            line_contents[0],
          );
          break;
        case "srad":
      ...
      <div>
        <Form.Item label="Choose the APBS input file">
          ...
          >
            <Button
              icon={<UploadOutlined />}
              disabled={!this.state.use_input_file}
            >
        ...
        <Form.Item label="Upload supporting files for the APBS input file above">
          ...
          >
            <Button
              icon={<UploadOutlined />}
              disabled={!this.state.show_apbs_misc_upload}
            >
  ...
  handleInfileUpload(info, self) {
    ...
    if (info.file.status === "done") {
      message.success(
        `APBS input file ${info.file.name} uploaded successfully`,
      );
    ...
    for (let line of lines) {
      ...
      if (READ_start && READ_end) {
        ...
        if (!line_stripped.startsWith("#")) {
          // Ignore comment lines of *.in file
  ...
  renderPqrAutofillUpload() {
    return (
      <Form.Item label="Autofill with PQR file">
        ...
        >
          <Button icon={<UploadOutlined />}>Click to Upload PQR File</Button>
        </Upload>
      </Form.Item>
    );
  }
  ...
  doubleUploadConfirm(file, self) {
    if (self.state.did_fetch) {
      ...
      return window.confirm(
        "Uploading a new file will overwrite ALL values within form. Continue?",
      );
    } else {
  ...
  renderCalcChoices() {
    ...
    let outputGroup = (
      ...
      <Radio.Group
        name={outputNameField}
        defaultValue={this.state.parent_form_values.type}
        onChange={this.handleParentFormChange}
        buttonStyle="solid"
      >
        {" "}
        {radioOptions}{" "}
  ...
  renderMethodFormItems() {
    console.log(
      "Calculation Method Type: " + this.state.parent_form_values.type,
    );
  ...
  renderCalcEnergy() {
    ...
    for (let optVal in outputOptions) {
      radioOptions.push(
        <Radio style={this.radioVertStyle} value={optVal}>
          {" "}
          {outputOptions[optVal]}{" "}
        </Radio>,
      );
    }
    let outputGroup = (
      <Radio.Group
        name={outputNameField}
        defaultValue={this.state.parent_form_values.calcenergy}
        onChange={this.handleParentFormChange}
      >
        {" "}
        {radioOptions}{" "}
  ...
  renderCalcForces() {
    ...
    let outputGroup = (
      <Radio.Group
        name={outputNameField}
        defaultValue={this.state.parent_form_values.calcforce}
        onChange={this.handleParentFormChange}
      >
        {" "}
        {radioOptions}{" "}
  ...
  renderOutputScalar() {
    ...
    for (let optVal in outputOptions) {
      checkboxOptions.push(
        <Checkbox style={this.radioVertStyle} name={optVal} value={optVal}>
          {" "}
          {outputOptions[optVal]}{" "}
        </Checkbox>,
      );
    ...
    let outputGroup = (
      <Checkbox.Group
        name={outputNameField}
        value={this.state.parent_form_values.output_scalar}
        onChange={(e) => this.handleParentFormChange(e, outputNameField)}
      >
        {" "}
        {checkboxOptions}{" "}
      </Checkbox.Group>
  ...
  renderOutputFormat() {
    ...
    let radioOptions = [];
    let outputOptions = { dx: "OpenDX", avs: "AVS UCD", uhbd: "UBHD" };
    ...
    for (let optVal in outputOptions) {
      ...
      radioOptions.push(
        <Radio disabled={disable_write_option} value={optVal}>
          {" "}
          {outputOptions[optVal]}{" "}
        </Radio>,
      );
    ...
    let outputGroup = (
      <Radio.Group
        name={outputNameField}
        value={this.state.parent_form_values.writeformat}
        onChange={this.handleParentFormChange}
      >
        {" "}
        {radioOptions}{" "}
  ...
  renderConfigForm() {
    return (
      <Form
        action={window._env_.API_URL + "/submit/apbs"}
        method="POST"
    ...
    return (
      <Form onSubmit={(e) => this.handleNewJobSubmit(e)} layout="vertical">
            ...
            >
              ...
              <TabPane
                ...
                tab={
                  <span>
                    <SettingOutlined />
                    {this.state.parent_form_values.type + " Options"}
                  </span>
                }
              ...
              >
                ...
                <Form.Item>
                  <Checkbox
                    ...
                    onChange={(e) =>
                      this.handleParentFormChange(
                        e,
                        "removewater",
                        !this.state.parent_form_values.removewater,
                      )
                    }
                  >
        ...
        <input
          type="hidden"
          name="hiddencheck"
          value={this.state.parent_form_values.hiddencheck}
        />
        <input type="hidden" name="pdb2pqrid" value={this.state.jobid} />
        <input
          type="hidden"
          name="mol"
          value={this.state.parent_form_values.mol}
        />
      </Form>
    );
  }
  ...
  renderConfigFormInfile() {
    ...
    if (
      this.state.v2_form_values.filename !== null &&
      this.state.v2_form_values.support_files.length > 0
    ) {
  ...
  render() {
    ...
    if (this.state.successful_submit) {
      ...
      if (this.props.jobid) {
        ...
        rendered_workflow = (
          <WorkflowHeader currentStep={2} stepList={WORKFLOW_TYPES.APBS} />
        );
      } else {
        rendered_config = this.renderConfigFormInfile();
        rendered_workflow = (
          <WorkflowHeader currentStep={0} stepList={WORKFLOW_TYPES.APBS_ONLY} />
        );
      }
    }
